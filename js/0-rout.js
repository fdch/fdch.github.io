...
// }
const randomSeed = ["sL7UX8IGJjWaoJ4",319201];
...
{
  // used in menus as onclick attribute
  let id = x.title;
  if (e.shiftKey) {
    getSubmit(id);
  } else {
...
{
  // used in menus as ontouch attribute
  let id = x.title;
...
// and returns an objet with keys and values 
// mapped to the original array

function mapToObj(arr) {

  return arr.map((v,i) => {
    return {k:i,v:v.toLowerCase()} 
  }).reduce((m, o) => {
    m[o.v] = o.k;
    return m;
  }, {});
